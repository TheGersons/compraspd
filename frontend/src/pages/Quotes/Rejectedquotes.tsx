...
export default function RejectedQuotes() {
    ...
    const cargarProductos = async () => {
        try {
            ...
            setProductos((data.items || []).map((p: any) => ({
                id: p.id,
                sku: p.sku,
    ...
    return (
        <>
            <PageMeta title="Cotizaciones Rechazadas" description="Productos rechazados en aprobación de compras" />
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">❌ Productos Rechazados</h1>
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Productos que fueron rechazados en la aprobación de compras
                    </p>
                </div>
