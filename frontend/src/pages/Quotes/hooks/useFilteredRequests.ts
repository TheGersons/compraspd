...
export const useFilteredRequests = (requests: AssignmentRequest[], filters: QuoteFilters) => {
  return useMemo(() => {
    const f = filters || {};
    const horizon =
      f.preset === "7d" ? 7 : f.preset === "30d" ? 30 : f.preset === "90d" ? 90 : null;
    ...
    const matchPreset = (r: AssignmentRequest) => {
      ...
      return daysUntil(r.quoteDeadlineISO) <= horizon; // incluye vencidas
      // si quieres excluir vencidas: d >= 0 && d <= horizon
    };
    ...
    const matchTipoCompra = (r: AssignmentRequest) => {
      ...
      const v = norm(String(r.procuremet)); // puede venir en mayúsculas
    ...
    const matchSearch = (r: AssignmentRequest) => {
      ...
      ].filter(Boolean).map(x => norm(String(x))).some(t => t.includes(q));
      return hay;
    };
