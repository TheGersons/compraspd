...
export class MailService {
  ...
  async sendNuevoSeguimientoNotification(
    ...
    const mailOptions = {
      from: `"Energía PD" <${process.env.MAIL_USER || 'noreply@energiapd.com'}>`,
      to,
      subject: `Nuevo seguimiento Import/Export: ${cotizacionNombre} - Energía PD`,
      html: `
        <!DOCTYPE html>
        <html lang="es">
        <head>
          <meta charset="utf-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Nuevo Seguimiento Import/Export</title>
        </head>
        <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f0f4f8; -webkit-font-smoothing: antialiased;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f0f4f8; padding: 48px 16px;">
            <tr>
              <td align="center">
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 520px; background: #ffffff; border-radius: 24px; overflow: hidden; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.1);">
                  <tr>
                    <td style="background: linear-gradient(145deg, #0D76B8 0%, #0a5a8c 50%, #14559c 100%); padding: 48px 40px; text-align: center;">
                      <div style="margin-bottom: 16px;">${this.getLogoSvg()}</div>
                      <p style="margin: 0; color: white; font-size: 13px; letter-spacing: 0.5px; text-transform: uppercase;">IMPORT / EXPORT</p>
                    </td>
                  </tr>
                  <tr>
                    <td style="padding: 48px 40px 32px;">
                      <h1 style="margin: 0 0 8px; color: #1a202c; font-size: 24px; font-weight: 700; text-align: center;">Nuevo Seguimiento</h1>
                      <p style="margin: 0 0 32px; color: #64748b; font-size: 15px; text-align: center; line-height: 1.6;">
                        Se ha creado un nuevo registro en el módulo <strong style="color: #334155;">Import/Export</strong>.
                      </p>
                      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 24px; margin-bottom: 32px;">
                        <table cellpadding="0" cellspacing="0" width="100%">
                          <tr>
                            <td style="padding: 8px 0; color: #64748b; font-size: 14px; border-bottom: 1px solid #e2e8f0;">
                              <strong style="color: #334155;">Cotización:</strong> ${cotizacionNombre}
                            </td>
                          </tr>
                          <tr>
                            <td style="padding: 8px 0; color: #64748b; font-size: 14px;">
                              <strong style="color: #334155;">Solicitante:</strong> ${solicitante}
                            </td>
                          </tr>
                        </table>
                      </div>
                      <div style="text-align: center; margin-bottom: 32px;">
                        <a href="${url}" style="display: inline-block; background: linear-gradient(135deg, #0D76B8 0%, #0a5a8c 100%); color: #ffffff; text-decoration: none; padding: 16px 48px; border-radius: 12px; font-weight: 600; font-size: 15px; box-shadow: 0 4px 14px rgba(13, 118, 184, 0.4);">
                          Ver Import/Export
                        </a>
                      </div>
                    </td>
    ...
    try {
      const info = await this.transporter.sendMail(mailOptions);
      this.logger.log(`Notificación import/export enviada a ${to}: ${info.messageId}`);
      return true;
    } catch (error) {
      this.logger.error(`Error enviando notificación import/export a ${to}:`, error);
      return false;
    }
  }
}
